...
namespace StS2AP.Utils
{
    ...
    public static class DeathLinkUtility
    {
        ...
        /// </summary>
        private static async Task AddCurseToDeck(string deathMessage)
        {
            ...
            try
            {
                ...
                {
                    var combatCard = (DeathLinkCurse)
                        GameUtility.CurrentPlayer.Creature.CombatState.CreateCard(
                            ModelDb.Card<DeathLinkCurse>(),
                            GameUtility.CurrentPlayer
                        );
            ...
            finally
            {
                ...
                if (DeathLinkDamagePercent > 0)
                {
                    ...
                    int damage = Mathf.RoundToInt(
                        GameUtility.CurrentPlayer.Creature.MaxHp * (DeathLinkDamagePercent / 100.0f)
                    );
                    newHp = Math.Max(0, GameUtility.CurrentPlayer.Creature.CurrentHp - damage);
                    TaskHelper.RunSafely(
                        CreatureCmd.SetCurrentHp(GameUtility.CurrentPlayer.Creature, newHp)
                    );
                }
                ...
                if (newHp > 0 && AreDeathFragmentsEnabled)
                {
                    ...
                    if (!deathMsg.Contains(info.Source))
                        deathMsg = $"{deathMsg} ({info.Source})";
